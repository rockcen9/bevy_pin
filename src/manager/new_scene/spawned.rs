...
fn rebuild_spawned_panel(
    ...
    let Some((container, _)) = containers.iter().find(|(_, c)| c.0 == "spawned-entities") else {
        return;
    };
...
fn spawned_item(entry: SpawnEntry) -> impl Scene {
    let label = format!("{} {}", entry.type_name, crate::utils::entity_display_label(entry.entity_id));
...
fn handle_spawned_item_click(
    ...
    for (interaction, item) in &items {
        if *interaction == Interaction::Pressed {
            debug!("spawned item clicked: {} entity #{}", item.type_name, item.entity_id);
