...
fn populate_pinboard(
    ...
    debug!("populate_pinboard: {} pending, {} new containers", pending.0.len(), added.iter().count());

    for container in &added {
        debug!("populate_pinboard: new ScrollableContainer key={}", container.0);
        let Some(idx) = pending.0.iter().position(|d| d.key == container.0) else {
            debug!("populate_pinboard: no pending match for key={}", container.0);
            continue;
        };
        let data = pending.0.remove(idx);
        debug!("populate_pinboard: matched entity_id={} highlight={}, setting Pinboard", data.entity_id, data.highlight);
        next_sidebar.set(SidebarState::Pinboard);

        if data.highlight {
            if let Some((title_entity, _)) = titles.iter().find(|(_, t)| t.0 == data.entity_id) {
                debug!("populate_pinboard: inserting PinCardHighlight on {:?}", title_entity);
                commands.entity(title_entity).insert(PinCardHighlight::new());
            } else {
                debug!("populate_pinboard: no PinCardTitle found for entity_id={}", data.entity_id);
            }
        }
    }
    ...
    pending.0.retain(|data| {
        ...
        debug!("populate_pinboard: existing card entity_id={} highlight={}", data.entity_id, data.highlight);
        next_sidebar.set(SidebarState::Pinboard);
        if data.highlight {
            debug!("populate_pinboard: inserting PinCardHighlight on existing {:?}", title_entity);
            commands.entity(title_entity).insert(PinCardHighlight::new());
        }
